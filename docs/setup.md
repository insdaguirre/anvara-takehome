...
4. ✅ Start Docker containers (PostgreSQL on port 5498)
...
CORS_ALLOWED_ORIGINS=http://localhost:3847
...
# OpenAI features (RAG + Agent)
OPENAI_API_KEY=
```

> **Note:** The setup script generates unique credentials automatically.
> **Production note:** `CORS_ALLOWED_ORIGINS` must be set explicitly in production; the backend fails closed if it is missing.
...
- ✅ **Verify seeded data** - See the demo sponsors, publishers, campaigns, and ad slots
- ✅ **Debug CRUD operations** - Check if your create/update/delete actually worked
- ✅ **Inspect relationships** - View how sponsors, campaigns, and other models connect
- ✅ **Test edge cases** - Manually modify data to test your error handling
...
1. ✅ Visit http://localhost:3847
2. ✅ Click "Login"
3. ✅ Log in with `sponsor@example.com` / `password`
4. ✅ You should see the sponsor dashboard
...
1. 📖 Read the [Challenges Overview](challenges/README.md)
2. 🏁 Start with [Challenge 1: TypeScript Errors](challenges/01-typescript.md)
3. 💾 Commit frequently as you work
